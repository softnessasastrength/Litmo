...
export default function QuizScreen() {
    ...
    profileRepository
      ...
      .then(({ draftProfile }) => {
        ...
        if (Array.isArray(saved)) {
          // Drop answers for questions that no longer exist (quiz length changed).
  ...
  const choose = (answer: QuizAnswer) => {
    ...
    persist(
      updated,
      isLast ? quizQuestions.length : index + 1,
...
function makeStyles(colors: AppColors) {
  return {
    ...
    header: { gap: 10, marginTop: 12, marginBottom: 24 },
    kicker: { color: colors.plum, fontSize: 15, fontWeight: "700" as const },
    prompt: {
      color: colors.ink,
