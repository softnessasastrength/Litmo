-- ADR 0035: permanent bans end open matching work.
-- matching_hold: keep cancelling only `requested` (029).
-- permanent_ban: also cancel pre-activation and safety-end active sessions.
...
begin
  ...
  -- Permanent ban only: close pre-activation and active sessions fail-closed.
...
comment on function public.apply_account_restriction(uuid, text, text, timestamptz, text) is
  'Staff restriction. Cancels requested sessions. Permanent ban also cancels pre-activation and safety-ends active sessions (ADR 0035).';
