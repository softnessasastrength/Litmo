...
- **Verifying true two-participant confirmation could not be completed in this session.** Sign-in is passkey-only (ADR 0010); there is no scripted way to register a second passkey identity or sign in as a different seeded account without a physical Face ID ceremony. This is documented plainly in `docs/LOCAL_DEVELOPMENT.md`'s "Demo accounts" section (which was also stale — it referenced password sign-in that no longer exists since ADR 0010) as the next manual step: either a second physical device, or manually registering a second passkey on the same device.
