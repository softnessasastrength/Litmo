...
Canonical snapshots are created only by a trusted server after `@litmo/domain` computation and are tied to exact immutable participant profile IDs and versions. `POST /api/sessions/:sessionId/snapshot` authenticates the participant, loads both latest version pairs server-side, rejects ineligible overlap, and persists through the service-only database function; clients cannot supply compatibility output. Each participant's confirmation row is readable only by that participant. A second exact-fingerprint confirmation moves the session to `ready`; no path can activate without both current confirmations. Pre-activation withdrawal requires no explanation, records the actor/time, clears confirmations, and cancels the session.

A material profile edit invalidates every affected pre-activation snapshot and clears its confirmations atomically. If the session was `ready`, it returns to `consent_pending`; it cannot activate until the trusted server creates a replacement from the latest exact versions and both people independently confirm that new fingerprint. Active and terminal session history is not retroactively changed.

Either participant may call the unified withdrawal authority before or during active state. It atomically clears confirmations and ends the session as `cancelled` or `soft_signaled`, accepts no reason, and returns a stable terminal result for duplicate identifiers. Mobile emergency stop disables locally and persists a reason-free pending action before network I/O. See ADR 0012 and `docs/CONSENT_WITHDRAWAL_AND_EMERGENCY_STOP.md`.
