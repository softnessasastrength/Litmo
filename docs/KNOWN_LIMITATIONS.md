...
- **Resolved 2026-07-12** (`docs/adr/0015-session-request-creation-and-recipient-authorization.md`): `request_session(...)` creates a session (user_a=requester/user_b=recipient), `transition_session(...)` requires the recipient specifically for `requested -> accepted`/`declined`, and the match screen / a new `app/app/requests.tsx` screen now reach these functions from the app for signed-in (non-demo) users. Not implemented: blocking/eligibility checks before request creation (no blocking system exists anywhere in this codebase), request expiration timestamps/jobs, and Realtime/push notification of a new incoming request (the recipient only sees it by opening the requests screen). Removal criterion: a blocking/eligibility subsystem, expiration timestamps/jobs, and Realtime/push delivery of new requests.
...
- Canonical snapshot persistence, trusted server computation/creation, exact-fingerprint confirmation, withdrawal, material profile-change invalidation, `ready -> active` enforcement, and participant-private wrap-up persistence now exist, and as of 2026-07-12 a real session can be requested, accepted, snapshot-confirmed by both participants, and activated entirely through the mobile app (`docs/adr/0015-session-request-creation-and-recipient-authorization.md`'s addenda). **Dual-participant confirmation has not been verified on-device**: sign-in is passkey-only (ADR 0010), and there is no scripted way to sign in as a second seeded account without a physical Face ID registration ceremony for it. Single-participant behavior (own confirmation recorded, honest "waiting for the other person" state) is reasoned through and code-correct but not click-tested; the two-participant path needs either a second physical device or manually registering a second passkey identity. Snapshot creation also depends on the separate Express backend being run locally and reachable on the same LAN as the phone (`EXPO_PUBLIC_BACKEND_URL`) — not viable for real-world use; moving that logic into a Postgres/Edge function is flagged but not decided. Transition-specific actor authorization (beyond the recipient-only accept/decline added in migration 015) remains otherwise unimplemented for other transitions. Mitigation: profile history is immutable, stale confirmation is cleared transactionally, activation remains database-gated, and wrap-ups are owner-only. Removal criterion: on-device dual-confirmation verification, a non-LAN-dependent snapshot-creation path, remaining transition actor-role enforcement, realtime recovery, wrap-up submission offline-retry, and two-client tests.
