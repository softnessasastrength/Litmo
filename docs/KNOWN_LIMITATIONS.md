...
- **Resolved 2026-07-12** (`docs/adr/0015`–`0019`, `0023`): session requests create/accept/decline/cancel; unanswered `requested` expires after 24h; pre-activation review (`accepted`/`consent_pending`/`ready`) expires 24h after review start; incoming Realtime + local alert. Not implemented: blocking/eligibility before request creation and remote OS push while backgrounded. Removal criterion: Chapter 5 block model + reviewed push delivery.
