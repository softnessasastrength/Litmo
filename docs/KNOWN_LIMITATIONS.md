...
- **Resolved 2026-07-12** (`docs/adr/0015`–`0019`, `0023`–`0029`): session requests create/accept/decline/cancel with 24h request + pre-activation expiry; Realtime + local alert; **one-way blocks**; **structured report intake**; **staff review queue**; **rate limits** on request/report/block thrash; **append-only trust events** + self-only signals (no universal score). Not implemented: remote OS push while backgrounded; consumer moderator console UI; account restrictions/appeals; auto-end of active sessions on block; peer-visible trust indicators. Removal criterion: reviewed push + restriction workflow + disclosure review.
