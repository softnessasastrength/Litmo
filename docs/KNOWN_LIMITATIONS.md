...
- **Resolved 2026-07-12** (`docs/adr/0015`–`0019`, `0023`–`0037`): session requests create/accept/decline/cancel with 24h request + pre-activation expiry; Realtime + local alert; **one-way blocks**; **structured report intake**; **staff review queue + console**; **rate limits**; **trust events** + self signals; **staff account restrictions** + appeals + lifecycle gates; **peer signals**; **real Consent Snapshot UI**; **staff-readable reporter messages + case evidence pack** (ADR 0037). Not implemented: remote OS push while backgrounded; auto-end of active sessions on block or matching_hold; full ops suite (search/bulk). Legacy device-encrypted report notes remain unreadable by staff. Removal criterion: reviewed push + disclosure review.
