...
- Canonical snapshot persistence, trusted server computation/creation, exact-fingerprint confirmation, withdrawal, material profile-change invalidation, `ready -> active` enforcement, and participant-private wrap-up persistence now exist. **Wrap-up and session-completion mobile wiring landed 2026-07-12** (`docs/adr/0014-wrap-up-mobile-wiring.md`): the wrap-up screen calls `submit_session_wrapup(...)` with the real five-outcome enum and an encrypted private note, and "End together" calls `transition_session(..., 'completed')` first. Transition-specific actor authorization remains unimplemented, and no caller yet threads a real `sessionId` into `/session/active` — session creation and request/accept flow don't exist yet, so this wiring is exercised by unit tests and is correct but currently dormant in the live demo path. Mitigation: profile history is immutable, stale confirmation is cleared transactionally, activation remains database-gated, and wrap-ups are owner-only. Removal criterion: session creation/request-accept flow that supplies a real `sessionId`, transition actor-role enforcement, realtime recovery, wrap-up submission offline-retry, and two-client tests.
