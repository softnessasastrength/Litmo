...
- **Resolved 2026-07-12** (`docs/adr/0015`–`0019`, `0023`–`0026`): session requests create/accept/decline/cancel with 24h request + pre-activation expiry; Realtime + local alert; **one-way blocks** hide discovery and reject requests opaquely; **structured report intake** with reporter-only status. Not implemented: remote OS push while backgrounded; human-review / moderator queue; auto-end of active sessions on block; rate limits. Removal criterion: reviewed push + moderator queue + abuse controls.
