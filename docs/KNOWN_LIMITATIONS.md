...
- **Resolved 2026-07-12** (`docs/adr/0015`–`0019`, `0023`–`0030`): session requests create/accept/decline/cancel with 24h request + pre-activation expiry; Realtime + local alert; **one-way blocks**; **structured report intake**; **staff review queue**; **rate limits**; **trust events** + self signals; **staff account restrictions** on discovery/requests. Not implemented: remote OS push while backgrounded; consumer moderator console UI; restriction gating of accept/transition; appeals; auto-end of active sessions on block; peer-visible trust indicators. Removal criterion: reviewed push + full restriction lifecycle + disclosure review.
