...
- **Resolved 2026-07-12 for request acceptance/decline** (`docs/adr/0015-session-request-creation-and-recipient-authorization.md`): `request_session(...)` now creates a session, establishing user_a=requester/user_b=recipient, and `transition_session(...)` requires the recipient specifically for `requested -> accepted`/`declined`. Every other transition still authorizes either participant, and no mobile screen calls `request_session` yet, so this is correct and tested but not yet reachable from the app. Not implemented: blocking/eligibility checks before request creation (no blocking system exists anywhere in this codebase) and request expiration timestamps/jobs. Removal criterion: a blocking/eligibility subsystem, expiration timestamps/jobs, and a mobile request/accept/decline UI.
