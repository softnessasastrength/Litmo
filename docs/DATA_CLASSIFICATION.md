...
| Shared Consent Snapshot | Exact profile IDs/versions, fingerprint, conservative compatibility output, withdrawal actor/time | Readable only by both session participants; server-created and deleted with the parent session. Never includes private nervous-system notes. Production retention remains undecided. |
