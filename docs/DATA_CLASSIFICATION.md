...
| Private session wrap-up | Enumerated comfort/safety outcome, optional private note, idempotency key | Owner-readable only and immutable after first submission. Never copied into the shared session audit trail. Deleted with the session under the current POC schema; safety-review routing and production retention remain undecided. |
