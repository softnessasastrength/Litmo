...
- **Installing `expo-build-properties` broke npm workspace hoisting** for `expo-router` badly enough to crash the Metro dev server (`Cannot find module 'expo-router/_ctx-shared'`) in this repo's monorepo layout. It was removed; the iOS deployment-target floor it would have set is instead handled by `app/plugins/withIOSXcode27Fixes.cjs`. Don't reintroduce it without re-testing Metro boots afterward.
