...
Server-backed participant reads on macOS currently include self-only trust history via `my_trust_signals` (ADR 0046), the owner-RLS own profile row (ADR 0047), and read-only pending session requests (ADR 0048). Shared transport and `ParticipantReadOutcome` keep fail-closed behavior consistent. Swift decodes and displays server fields; it does not recompute trust scores or consent, and does not mutate requests. Configuration and session absence fail closed. Passkey sign-in and broader participant read models remain future reviewed work.
