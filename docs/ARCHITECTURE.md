...
Migration `007_session_lifecycle.sql` (`agent/chapter-4-session-lifecycle` branch) aligns the Chapter 1/2 `sessions` table's status values with this same canonical list and adds `session_events`, an append-only audit trail table. Both are read-only for `authenticated` participants so far — no client can write `sessions.status` or append an event directly. Actor authorization, snapshot-version matching at `ready -> active`, idempotency-key deduplication, and the transition function that will be the only path to writing these rows remain explicit follow-up work (see ADR 0005's "Update" section).
