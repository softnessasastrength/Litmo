...
Server-backed participant reads on macOS currently include self-only trust history (ADR 0046), own profile (ADR 0047), read-only pending session requests (ADR 0048), and self export via `export_my_data` (ADR 0049). Shared transport and `ParticipantReadOutcome` keep fail-closed behavior consistent. Swift decodes and displays server fields; it does not recompute trust scores or consent, and does not mutate requests. Configuration and session absence fail closed. Passkey sign-in and broader participant read models remain future reviewed work.
