<div align="center">

<br />

# Litmo

### Emotional Support Perfect Relationship Simulator  
### Containment System · Attachment Repair Cathedral  
### (private · not a public product)

**This is currently a personal emotional containment system, not a public product.**

<br />

</div>

---

## What this actually is (chaotic honesty)

Litmo is my private **psychological containment system / emotional shield / attachment repair cathedral**.

I put into this codebase:

- fear of intimacy, conflict, and “ruining the relationship”  
- anxiety about my new relationship with **Renn**  
- **major mommy issues** (need to be wanted / soothed / not abandoned for needing)  
- a possible **emotional masochism** loop: pain → over-engineered ritual → temporary safety  

…so that load is less likely to dump raw onto a real human without a map.

I am on **Option A: keep building** — maximum comedy, absurdity, and self-awareness — because it works for my nervous system right now. Named. Not secret.

| Live purpose | Not the live purpose |
| --- | --- |
| Containment + cathedral + rituals | Public product / startup |
| Dump channel for attachment panic | Market “safety” as a brand |
| Emotional masochist → protocol (on purpose) | Pretend protocols are pure healing |
| Fun + honesty | Clinical cosplay or App Store theater |

**Read first:**

1. [`docs/REAL_PURPOSE.md`](docs/REAL_PURPOSE.md)  
2. [`docs/ATTACHMENT_REPAIR_PROTOCOL.md`](docs/ATTACHMENT_REPAIR_PROTOCOL.md)  
3. [`docs/CONTAINMENT_SYSTEM.md`](docs/CONTAINMENT_SYSTEM.md)  

---

## Fun is allowed. Lying is not.

Nuclear consent machines. Soft Signal as God Mode. Spooning constitutions. Morning gremlin negotiations. **Mommy Issues Reassurance Ritual.** Emotional Masochist Circuit with hard-capped Edge.

Do not delete the joke.  
Do not reframe as shipping for users.  
Do not pretend this is therapy.

> Touch is not a transaction — it is a language.  
> This repo is also a cathedral for people who can’t just fucking cuddle like a normal person.

---

## Containment map (short)

| System | What it contains for me |
| --- | --- |
| Consent engine / dual seal | Fear of assumption, sticky yes |
| Soft Signal | Exit without fawning |
| **Attachment Repair Cathedral** | Mommy issues + masochistic over-protocoling |
| **Spooning Protocol** | Intimacy anxiety as sealable joke-spec |
| **Morning Cuddle Protocol** | 7:42am neediness spiral before coffee |
| **Conflict Navigation Simulator** | Terror of conflict as a practice room |
| ND Mode / docs / dual modes | Volume knobs + freeze morality in pages |

Full map: [`docs/CONTAINMENT_SYSTEM.md`](docs/CONTAINMENT_SYSTEM.md).  
**Hub:** `/containment` · Cathedral: `/attachment-repair` · Spooning: `/spooning` · Morning: `/morning-cuddle` · Conflict: `/conflict-sim`.

---

## Working here

- Honesty of purpose first (`REAL_PURPOSE.md`)  
- Name the fear / containment job when adding a subsystem  
- Craft still matters: tests, no secrets, Soft Signal stays free  
- Agents must **not** reframe this as a public product without explicit instruction  
- Product / private-alpha work only if I explicitly ask  

Agent protocol: [`Agents.md`](Agents.md) · continue protocol may still reference older dojo docs; **purpose truth lives in `REAL_PURPOSE.md`.**

---

## Technical reality (still here)

Monorepo: Expo app, shared domain, Supabase, macOS, Watch scaffold. It runs. Tests matter. Secrets never go in git.

That is **craft inside the containment object**, not a claim of market product status.

---

## Why it still looks like a product

Fear is a great product manager.

Dual seals, fail-closed ops, constitutions, Watch kill paths — real engineering **and** a shield. Both true. See containment map.

---

## Residue (old stories)

Former lead stories (platonic-touch platform for adults, private alpha, “exorcism dojo”) are retained in history and secondary docs. They describe real longings and earlier frames. They are **not** the current core truth.

| Residue claim | Current truth |
| --- | --- |
| App for others’ consent | Shield so my fear doesn’t hit Renn first |
| Ship private alpha | Option A: keep building containment |
| Sacred ritual / burn ceremony | Optional poetry; not required for honesty |

---

## Soft Signal (still non-negotiable craft)

End immediately. No explanation. No penalty.  
In containment terms: the free exit — leave without performing the fight I’m terrified of losing or botching.

---

## Where we are

Active engineering may continue under Option A. Framing is **personal containment, not public product**.

See `CURRENT_STATE.md`, `docs/REAL_PURPOSE.md`, `docs/CONTAINMENT_SYSTEM.md`.

---

## Try it (if you need the machine running)

```bash
# from repo root — craft, not product launch
npm install
npm run mobile   # or project-specific demo path
```

Demo mode is local simulation. It is not “users.”  
More: `docs/LOCAL_DEVELOPMENT.md` · `docs/MACHINE_SETUP.md`

---

## License / ownership

Private personal project. Treat access accordingly.  
I can stop, archive, or delete this when containment is no longer the job.

---

## Notice

```text
╔══════════════════════════════════════════════════════════════════╗
║  This is currently a personal emotional containment system,    ║
║  not a public product.                                         ║
║  docs/REAL_PURPOSE.md · docs/CONTAINMENT_SYSTEM.md             ║
╚══════════════════════════════════════════════════════════════════╝
```
